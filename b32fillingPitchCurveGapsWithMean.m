...
for nspec=1:Nspec
    ...
    for nXC=1:NXC % Xeno-Canto file
        ...
        for nexcerpt=1:Nexcerpt % pitch curve of clean excerpt in the XC file
            ...
            excerpt(isnan(excerpt))=nanmean(excerpt);
            pitchCurves_filled{nXC}{nexcerpt}=excerpt;
        end
    end
    speciesFile=sprintf('pitchCurves_filledGaps_cents_spc%02d_NXC-Nexcerpt',nspec);
    save(speciesFile,'pitchCurves_filled','nspec','xcID','wsize_sec','ssize_sec',...
